...
let fn_emojiElementsToPanel = (type, panel, quill) => {
...
let fn_emojiPanelInit = (panel, quill) => {
...
let fn_updateEmojiContainer = (emojiFilter, panel, quill) => {
...
let fn_showEmojiPalatte = quill => {
    ...
    emojiType.map(emojiType2 => {

    // add tab bar
        let tabElement = document.createElement('li');

        tabElement.classList.add('emoji-tab');
        tabElement.classList.add(`filter-${emojiType2.name}`);
        let tabValue = emojiType2.content;

        tabElement.innerHTML = tabValue;
        tabElement.dataset.filter = emojiType2.type;
        tabElementHolder.appendChild(tabElement);

        let emojiFilter = document.querySelector(`.filter-${emojiType2.name}`);
...
class ToolbarEmoji extends Module {
    ...
    constructor (quill, options) {
        ...
        let emojiBtns = document.getElementsByClassName('ql-emoji');
