...
let computeBoundaryFromRects = (startRect, endRect) => {
...
export default class TableSelection {
    ...
    mouseDownHandler (e) {
        ...
        let mouseMoveHandler = evt => {
            ...
            if (evt.button !== 0 || !evt.target.closest('.quill-better-table')) {

                return;
            
            }

            const endTd = evt.target.closest('td[data-row]');
    ...
    repositionHelpLines () {
        ...
        const parentViewScrollTop = this.quill.root.parentNode.scrollTop;

        css(this.left, {
            display : 'block',
            left : `${this.boundary.x - tableViewScrollLeft - 1}px`,
            top : `${this.boundary.y + (parentViewScrollTop * 2)}px`,
        ...
        css(this.bottom, {
            display : 'block',
            left : `${this.boundary.x - 1 - tableViewScrollLeft}px`,
            top : `${this.boundary.y1 + 1 + (parentViewScrollTop * 2)}px`,
            width : `${this.boundary.width + 1}px`,
            height : '1px'
        });
