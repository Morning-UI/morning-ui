<template>
    ...
    下拉选择框的选项通过`list`配置传入，`list.val`是选项的值，`list.name`是选项的名称(支持HTML)。
    ...
    - 小于`200`项：不需要特殊优化
    - `200`至`1000`项：采用[max-show](#max-show)配置优化，可减少DOM数量
    - 大于`1000`项：采用[dynamic-list](#dynamic-list)配置优化，可减少`list`中的数据及DOM数量，同时也可采用远程查询。
